...
class Mastermind{
    ...
    public function validar($numero){
        if(in_array($numero, $this->jugadas['codigo'])){
    ...
    public function compruebaNumero($numero){
        ...
        array_push($this->jugadas['codigo'], $numero);
    ...
    public function getUltimaJugada(){
        $ultimaJugada = count($this->jugadas['codigo']) - 1;
        return array_slice($this->jugadas['codigo'], $ultimaJugada);
    }
}
